...
<head>
  ...
  <title>تفاصيل الجهاز - Amancom</title>
...
<body>
  ...
  <section class="profile-details">
    <h2>بيانات الجهاز</h2>
    <p><strong>رقم الجهاز:</strong> <span id="serial_number">جار التحميل...</span></p>
    <p><strong>نوع الجهاز:</strong> <span id="device_type">جار التحميل...</span></p>
    <p><strong>الحالة:</strong> <span id="status">جار التحميل...</span></p>
    <p><strong>تاريخ الشراء:</strong> <span id="purchase_date">جار التحميل...</span></p>
  ...
  <script>
    ...
    fetch(`../api/get_device_details.php?id=${serial}`)
      .then(res => res.json())
      .then(data => {
        if (data.error) return alert(data.error);
        const dev = data.device;
        document.getElementById('serial_number').textContent = dev.serial_number;
        document.getElementById('device_type').textContent = dev.device_type;
        document.getElementById('status').textContent = dev.status;
        document.getElementById('purchase_date').textContent = dev.purchase_date;
        ...
        if (data.customer) {
          ...
          actionsArea.innerHTML = `<button class="unlink-btn" onclick="unlinkDevice()">❌ إلغاء الربط</button>`;
        } else {
      ...
      fetch(`../api/search_clients.php?query=${encodeURIComponent(keyword)}`)
        ...
        .then(results => {
          resultBox.innerHTML = results.map(c => `
            <li data-id="${c.customer_id}">${c.name} - ${c.contact_info}</li>
          `).join("");
        });
    });

    document.getElementById("clientResults").addEventListener("click", function(e) {
      if (e.target.tagName === "LI") {
        const customerId = e.target.dataset.id;
        bindDeviceToClient(customerId);
      }
    });

    function bindDeviceToClient(customerId) {
      fetch("../api/link_device_to_customer.php", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
        body: "serial_number=" + encodeURIComponent(serial) + "&customer_id=" + encodeURIComponent(customerId)
      })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
    ...
    function unlinkDevice() {
      if (!confirm("هل أنت متأكد من إلغاء ربط الجهاز بالعميل؟")) return;
      fetch("../api/unlink_device.php", {
        method: "POST",
        headers: { "Content-Type": "application/x-www-form-urlencoded" },
